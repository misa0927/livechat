...
func Test_UserDelete(t *testing.T) {
  ...
  if err := users[0].Delete(); err != nil {
    t.Error(err, "- Cannnot delete user")
  }
...
func Test_UserByUUID(t *testng.T) {
  setup()
  if err := users[0].Create(); err != nil {
    t.Error(err, "Cannot create user.")
  }
...
func Test_Users(t *testing.T) {
  ...
  u, err := Users()
  if err != nil {
    t.Error(err, "Cannnot retrieve users.")
  }
  if len(u) != 2 {
...
func Test_GetSession(t *tensting.T) {
  setup()
  id err := users[0].Create(); err != nil {
    t.Error(err, "Cannnot create user.")
  }
  session, err := users[0].CreateSession()
  if err !=nil {
    t.Error(err, "Cannot create session")
  ...
  if err != nil {
    t.Error(err, "Cannnot get session")
  }
  if s.Id == 0 {
...
func Test_checkValidSession(t *testing.T) {
  ...
  valid, err := s.Check()
  if err != nil {
    t.Error(err, "Cannnot check session")
  }
  if valid != true {
...
func Test_DeleteSession(t *testing.T) {
  ...
  session, err := users[0].CreteSession()
  if err != nil {
    t.Error(err. "Cannot crete session")
  }

  err = session.DeleteByUUID()
  if err != nil {
    t.Error(err, "Cannot delet session")
  }
  s := Session{Uuid: session.Uuid}
  valid, err := s.Check()
  if err == nil {
    t.Error(err, "Session is vaild even though deleted")
  }
  if valid == true {
